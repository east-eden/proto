...
proto:
	...
	# 客户端导出
	protoc -I=./ --csharp_out=:../ee_client/proto --csharp_opt=base_namespace=proto ./global/*.proto

.PHONY: linux_proto
linux_proto:
	# 服务器导出
	./bin/protoc_server -I=./ --go_out=:../server/proto --micro_out=:../server/proto --go_opt=module=bitbucket.org/funplus/server/proto --micro_opt=paths=source_relative ./global/*.proto
